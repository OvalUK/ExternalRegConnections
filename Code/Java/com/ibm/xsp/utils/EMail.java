...
public class EMail {
	...
	public static EMail get() {
		...
		EMail bean = (EMail) context.getApplication().getVariableResolver()
	...
	private Database getCurrentDatabase() throws NotesException {
		return getCurrentSession().getCurrentDatabase();
	}
	...
	public void send() throws NotesException, IOException, Exception {
		...
		if (null != session && null != database && null != this.sendTo
			...
			try {
									...
									if (null != emailRootChild
										...
										try {
											...
											if (persistentName == null)
												persistentName = ((AttachmentValueHolder) attachment)
														.getName();
				...
				if (saveEmail) {
					emailDocument.replaceItemValue("Form", "Memo");
					emailDocument.save();
				}
